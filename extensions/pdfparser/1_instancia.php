...
function insere($texto)
{
	//echo $texto."<br>";
	global $connect;
	$sql =  "SELECT * FROM 1_instancia";
	$resultado = mysqli_query($connect, $sql);	
	$dados = mysqli_fetch_array($resultado);
	$data = date("Y/m/d");
	
	$sql = "INSERT INTO 1_instancia(data, texto) VALUES ('$data', '$texto')";
...
//Cria um arquivo de texto para 1° instância
function criaArquivo(){

	//Verifica se o arquivo existe
	if(file_exists("arquivos/1_instancia".str_replace("/", "", date("d/m/Y")))):
		die();
	else:
		
		//Salva a url de acordo com a data
		$url = "https://esaj.tjms.jus.br/cdje/downloadCaderno.do?dtDiario=".date("d/m/Y")."&cdCaderno=3&tpDownload=D";
		...
		foreach ($pages as $page) {
		    ...
		    //Cria um arquivo txt
			$arquivo = fopen("arquivos/1_instancia/".str_replace("/", "", date("d/m/Y")), "a");
...
switch (date("l")) {
	case 'Monday':
		criaArquivo();
		filtroString("VANESSA DE JESUS OLIVIO", "ADV: ", "arquivos/1_instancia/".str_replace("/", "", date("d/m/Y")));
		break;
